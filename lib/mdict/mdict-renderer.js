...
(function (root, factory) {
  ...
  if (typeof define === 'function' && define.amd) {
    ...
    define(['./jquery-1.11.3.min', './bluebird', './pcmdata.min', './bitstring.min'], factory);
  } else {
    // Browser globals
    factory(jQuery, Promise);
  }

}(this, function ($, Promise, PCMDataLib) {
  var MIME = {
    'css': 'text/css',
    'img': 'image',
    'jpg': 'image/jpeg',
  ...
  return function createRenderer(resources) {
    ...
    function replaceImage(index, img) {
      ...
      cache.get(src, loadData.bind(null, MIME['img']))
           .then(function(url) {
              $img.attr({ src: url, 'data-old-src': src });
            });
    }
    ...
    function renderAudio() {
      ...
      if ($a.attr('href_')) {
        ...
        cache.get(res, loadAudio.bind(null, ext))
             .then(function(url) {
                $a.append($('<audio>').attr({src: url, 'data-old-src': href})).on('click', playAudio);
